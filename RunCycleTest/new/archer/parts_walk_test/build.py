...
def pose(t):
    ...
    node('pelvis',None,[310,345+bob],0,z=40)
    node('torso','pelvis',[0,-7],-.025*c,z=45)
    ...
    for side,shift,z in [('far',.5,15),('near',0,70)]:
        ...
        node(side+'_upper_arm','torso',[(-19 if side=='far' else 14),-104],swing,z=z+5)
    ...
    return {'phase':phase,'surface':variant,'nodes':nodes}
def world(p):
    result={}
    def resolve(name):
...
rig={'parts':parts,'samples':samples,'cycle_seconds':1.2,'ground_y':566,'canvas':[640,640],'key_samples':[0,30,60,90],'note':'Angles radians, positive clockwise; local parent-child transforms. Variant front/rear switches explicitly; head fixed right. No identity swaps.'}
...
metrics={'samples':N,'max_joint_length_error_px':max(errors),'max_stance_ankle_height_error_px':max(support),'max_bow_grip_attachment_error_px':max(bow),'loop_endpoint_equal':pose(0)==pose(1),'near_foot_contact_x':[world(samples[k])['near_foot']['xy'][0] for k in [0,60]],'near_hand_contact_x':[world(samples[k])['near_hand']['xy'][0] for k in [0,60]],'warning':'Attachment metrics validate coordinate anchors, not painted grip or pixel-level joint coverage. Front/rear silhouettes differ and switch abruptly.'}
