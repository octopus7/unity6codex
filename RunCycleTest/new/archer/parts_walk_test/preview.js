...
function sample(v){
 ...
 if($('surface').value!=='auto'&&!['head','cape','quiver','bow'].includes(k))nodes[k].art=$('surface').value+'/'+k}
...
function draw(){
 ...
 for(let [k,n] of Object.entries(p.nodes).sort((a,b)=>a[1].z-b[1].z)){if(k==='cape'&&!$('cape').checked)continue;let w=world[k],m=RIG.parts[n.art];ctx.save();ctx.translate(...w.xy);ctx.rotate(w.a);ctx.drawImage(imgs[n.art],-m.pivot[0],-m.pivot[1],...m.size);ctx.restore()}
